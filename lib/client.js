...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // 侧栏入口开关：注册进插件集的插件**默认不进侧栏**，由用户在面板里逐个开启
    const SIDEBAR_PREF_KEY = "dsh-plugin-hub:sidebar-entries";
    function readSidebarPrefs() {
      try { return JSON.parse(localStorage.getItem(SIDEBAR_PREF_KEY) || "{}") || {}; } catch (e) { return {}; }
    }
    function writeSidebarPrefs(prefs) {
      try { localStorage.setItem(SIDEBAR_PREF_KEY, JSON.stringify(prefs)); } catch (e) { /* ignore */ }
    }
    function ensureRegistry() {
      const existing = typeof window !== "undefined" ? window[HUB_KEY] : null;
      if (existing && typeof existing.register === "function" && typeof existing.setSidebarOn === "function" && existing.version >= 2) return existing;
      const entries = [];
      ...
      const api = {
        ...
        /** 该插件是否在侧栏显示独立入口（默认 false） */
        sidebarOn(id) { return !!readSidebarPrefs()[id]; },
        setSidebarOn(id, on) {
          const prefs = readSidebarPrefs();
          if (on) prefs[id] = true; else delete prefs[id];
    ...
    const S = {
      ...
      toggle: (on) => ({ flex: "0 0 auto", cursor: "pointer", border: "1px solid " + (on ? "var(--dsw-alias-label-primary-bluish, #6b8fd4)" : V.border), borderRadius: "4px", background: "transparent", color: on ? "var(--dsw-alias-label-primary-bluish, #6b8fd4)" : V.textDim, padding: "0 5px", fontSize: "10px", lineHeight: "16px", whiteSpace: "nowrap" }),
    ...
    function Panel(props) {
      ...
      const side = h("div", { style: S.side },
        ...
        entries.length ? h("div", { style: { fontSize: "10px", color: V.textDim, padding: "0 8px 4px", lineHeight: 1.5 } }, "注册进来的插件默认不进侧边栏；点「侧栏」才在侧边栏显示独立入口。") : null,
        entries.length === 0
          ? h("div", { style: { fontSize: "11px", color: V.textDim, padding: "4px 8px" } }, "（暂无插件注册面板）")
          : entries.map((e) => {
              const sidebarOn = hub.sidebarOn(e.id);
              return h(HoverRow, {
                key: e.id, title: e.title || e.id,
                on: active && active.kind === "page" && active.id === e.id,
                onClick: () => setActive({ kind: "page", id: e.id })
              },
                h("span", { style: { flex: "0 0 auto" } }, e.icon || "▫"),
                h("span", { style: { flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis" } }, e.title || e.id),
                h("button", {
                  type: "button",
                  title: sidebarOn ? "从侧边栏移除独立入口" : "在侧边栏显示独立入口",
                  style: S.toggle(sidebarOn),
                  onClick: (ev) => { if (ev && ev.stopPropagation) ev.stopPropagation(); hub.setSidebarOn(e.id, !sidebarOn); }
                }, sidebarOn ? "侧栏 ✓" : "侧栏")
              );
            }),
    ...
    function Dock(props) {
      ...
      // 默认只显示「插件集」一个入口；其余插件要在面板里把「侧栏」打开才出现
      const docked = entries.filter((e) => e.dock !== false && hub.sidebarOn(e.id));
